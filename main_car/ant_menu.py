...
class Menu:
    def __init__(self, flash_sys, beep, lcd, key_data, key_handler):
        # 所有参数强制转为浮点数，避免类型错误
        self.config = {
            # PID 参数
        ...
        self.param_short_name = {
            ...
            # 砖块数量、角点 & 凸起中心
            "block_count": "b_cnt",
            "block1_corner1": "b1_c1",
            "block1_corner2": "b1_c2",
            "block2_corner1": "b2_c1",
            "block2_corner2": "b2_c2",
            "block3_corner1": "b3_c1",
            "block3_corner2": "b3_c2",
            "bump_center": "bump_ct",
        }
        gc.collect()
        # 注入外部硬件对象
        ...
        self.beep = beep
        self.lcd = lcd
        self.key_data = key_data            # 五向开关数据（5元素数组，用户填写引脚接口）
        self.key_handler = key_handler
        self.key_index_map = {"up":0, "down":1, "left":2, "right":3, "press":4}
        ...
        # 标题偏移：str24标题(24px) + 分隔线(2px) + 间距(4px) = 30px
        # 比原来 str16 标题(16px) 多占 14px
        self.TITLE_EXTRA = 14
    ...
    def _init_core_mappings(self):
        """初始化核心映射（8页分区，BLOCK为第1页）"""
        self.page_line_map = {
            # 1.BLOCK页（砖块数量、角点 & 凸起中心）
            1: {
                2: ("block_count", "6.3f"),
        ...
        # 页面-保存行映射
        self.save_line_map = {
            1: 10,   # BLOCK：8个参数 → 10行保存
            2: 11,   # PID：9个参数 → 11行保存
            3: 4,    # COEF：2个参数 → 4行保存
            4: 7,    # TIME：5个参数 → 7行保存
            5: 4,    # PATH：2个参数 → 4行保存
            6: 10,   # SPEED：8个参数 → 10行保存
            7: 15,   # SERVO：13个参数 →15行保存
            8: 5     # ORBIT：3个参数 →5行保存
        }
        ...
        # 页面-标题-行范围映射
        self.page_meta = {
            1: ("BLOCK", 1, 10),  # BLOCK：8参数+步长+保存 → 1-10行
            2: ("PID", 1, 11),    # PID：9参数+步长+保存 → 1-11行
            3: ("COEF", 1, 4),    # COEF：2参数+步长+保存 →1-4行
            4: ("TIME", 1, 7),    # TIME：5参数+步长+保存 →1-7行
            5: ("PATH", 1, 4),    # PATH：2参数+步长+保存 →1-4行
            6: ("SPEED", 1, 10),  # SPEED：8参数+步长+保存 →1-10行
            7: ("SERVO", 1, 15),  # SERVO：13参数+步长+保存 →1-15行
            8: ("ORBIT", 1, 5)    # ORBIT：3参数+步长+保存 →1-5行
        }
    ...
    def refresh_param_line(self, line_num, config_key):
        ...
        # 清空当前行
        self.lcd.str16(0, y, self.CLEAR_SPACES, 0x0000)
        # 绘制格式化后的参数（完整显示三位小数）
        display_text = self._format_param_display(config_key)
        self.lcd.str16(5, y, display_text, 0xFFFF)
    ...
    def _redraw_current_arrow(self):
        ...
        self.lcd.str16(self.ARROW_X, self.TITLE_EXTRA + self.LineSpacing * self.Current_line, "<--", arrow_color)

    # ========== 读取五向开关输入 ==========
    def read_key(self):
        """读取五向开关所有方向（用户自行填充 key_data 引脚读取逻辑）"""
        pressed_key = None

        # press（按下）优先级最高
        press_idx = self.key_index_map["press"]
        if self.key_data[press_idx] == 1:
            self.key_handler.clear(press_idx + 1)
            self.beep.key_test()
            return "press"

        # 读取 up / down / left / right
        if self.key_data[self.key_index_map["up"]] == 1:
            self.key_handler.clear(self.key_index_map["up"] + 1)
            self.beep.key_test()
            pressed_key = "up"
        elif self.key_data[self.key_index_map["down"]] == 1:
            self.key_handler.clear(self.key_index_map["down"] + 1)
            self.beep.key_test()
            pressed_key = "down"
        elif self.key_data[self.key_index_map["left"]] == 1:
            self.key_handler.clear(self.key_index_map["left"] + 1)
            self.beep.key_test()
            pressed_key = "left"
        elif self.key_data[self.key_index_map["right"]] == 1:
            self.key_handler.clear(self.key_index_map["right"] + 1)
            self.beep.key_test()
            pressed_key = "right"
    ...
    def refresh_current_page(self):
        ...
        for i in range(self.Start_line, self.End_line + 1):
            y = self.TITLE_EXTRA + self.LineSpacing * i
            self.lcd.str16(self.ARROW_X, y, "   ", self.COLOR_WHITE)
    ...
    def data_processing(self, key):
        ...
        if self.Current_line == 1:
            ...
            if old_index != self.current_step_index:
                sy = self.TITLE_EXTRA + self.LineSpacing * 1
                self.lcd.str16(0, sy, self.CLEAR_SPACES, 0x0000)
                step_text = f"step : {self.step_values[self.current_step_index]:8.3f}"
                self.lcd.str16(5, sy, step_text, 0xFFFF)
                self._redraw_current_arrow()
            gc.collect()
            return
    ...
    def move_arrow(self, key):
        """移动箭头位置（移动时取消参数选中）"""
        # 清空当前箭头
        self.lcd.str16(self.ARROW_X, self.TITLE_EXTRA + self.LineSpacing * self.Current_line, "   ", self.COLOR_WHITE)
    ...
    def _show_page(self, page_num):
        """通用页面绘制方法（str24标题+分隔线+三位小数参数）"""
        title, start_line, end_line = self.page_meta.get(page_num, ("", 1, 14))
        self.Start_line, self.End_line = start_line, end_line

        # 全屏清除后绘制标题（str24大字体，动态居中）
        self.lcd.clear(0x0000)
        title_x = max(0, (self.LCD_WIDTH - len(title) * 14) // 2)  # str24每字约14px
        self.lcd.str24(title_x, 0, title, 0xFFFF)
        # 标题下方画分隔线
        self.lcd.line(0, 26, self.LCD_WIDTH, 26, 0xFFFF, 1)
        ...
        if save_line:
            sy = self.TITLE_EXTRA + self.LineSpacing * save_line
            self.lcd.str16(0, sy, self.CLEAR_SPACES, 0x0000)
            self.lcd.str16(5, sy, "save", 0xFFFF)
