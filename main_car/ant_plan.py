...
class PathPlan:
    ...
    def plan_path(self, x1, y1, ignore_center_rect=False, start_point = []):
        ...
        def get_nearest_valid(p):
            ...
            valid_p = (px, py)
            if self._point_valid(valid_p, circles, rects, block_r):return valid_p
        ...
        # 检查直连
        if self._line_valid(start, end, circles, rects, block_r):self.ready_path = [[float(end[0]), float(end[1])]]
        # 初始化节点列表
        nodes = [start, end]
        # 添加圆形中继点 (核心修改部分)
