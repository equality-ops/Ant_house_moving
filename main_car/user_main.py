...
# 创建路径规划对象
my_path_planner = ant_plan.PathPlanner()
...
# 任务及类
my_task = ant_else.TaskController(my_beep, my_fan, my_photo, my_state, my_uart3, my_uart8, my_car, my_plan, my_vision_manager, plan_data, my_order_manager, my_art_protocol,  my_main_protocol, my_path_planner)
...
def show_speed_PID_test():
    global counter
    # counter += 1
    # motor_ul_pid.compute_pid(50, pose_data.encoder_data_ul)
    # motor_ur_pid.compute_pid(180, pose_data.encoder_data_ur)
    # motor_md_pid.compute_pid(110, pose_data.encoder_data_md)
    ...
    if counter >= 2000:
        ...
        motor_ul_pid.compute_pid(160, pose_data.encoder_data_ul)
    else:
        motor_ul_pid.compute_pid(30, pose_data.encoder_data_ul)
    """
...
def test_preview():
    if my_state.state == READY_NAVIGATE:
        ...
        my_order_manager.mode_preview()
    elif my_state.state == PREDICT:
        obj_message = my_art_protocol.preview_receive()
        if obj_message :
            # predict_pt = my_vision_manager.predict_point(target_point[0], target_point[1])
            my_uart3.write(f"obk: {obj_message}\n")
            my_state.state = STOP
...
def time_pit2_handler(time):
    ...
    # my_uart3.write(f"{my_plan.aimed_point_index}\r\n")
    # my_uart3.write(f"{my_task.scan_message}, {my_state.state}\r\n")
