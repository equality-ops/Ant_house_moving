...
class beep:
    ...
    # 长按按键测试函数(响两声，每80ms响一声，每次持续80ms)
    def long_key_test(self) -> None:
        if self.beep_state == BEEP_OFF:
            self.beep_state = BEEP_ON
            self.beep.high()
            time.sleep_ms(80)
...
class TaskController:
    def __init__(self, flash_sys: flash_system, beep: beep, fan, photo, state, uart3, uart8, car, plan, vision, plan_data, order_manager: order_manager, art_protocal: UARTProtocol, main_protocol: LinkProtocol):
        ...
        # 任务模式
        self.task_mode = TASK_NORMAL  # 默认任务模式为正常模式
        ...
        self.move_direction = D_TO_D  # 搬运方向
        # 标志位
    ...
    def reset_car_pos(self):
        # 经验修正值
        correction = 2.0
        if self.move_direction in [D_TO_D, U_TO_D]:
            self.my_car.y_current = 0.0 + correction
        elif self.move_direction in [D_TO_U, U_TO_U]:
            self.my_car.y_current = self.data.FIELD_H - correction

    # 修改物体状态
    def change_move_direction(self):
        def change():
            self.if_change_status = True
            if self.task_mode == TASK_NORMAL:
                ...
                if self.move_direction == D_TO_U:
                    self.data.scan_point[0] = self.data.scan_path_1[1]
                    del self.data.scan_path_1[:2]
                elif self.move_direction == U_TO_D:
                    self.data.scan_point[1] = self.data.scan_path_2[1]
    ...
    # 输入一个坐标点，返回以该点为中心的矩形的四个顶点列表
    def get_rect_vertices(self, center, half_length, half_width):
        """
        Args:
            center: 中心坐标点，格式为 (x, y)
            half_length: 矩形长的一半
            half_width: 矩形宽的一半

        Returns:
            四个顶点的列表，格式为 [(x1,y1), (x2,y2), (x3,y3), (x4,y4)]
            顶点顺序：左上、右上、右下、左下
        """
        cx, cy = center
        vertices = [
            (cx - half_length, cy - half_width),  # 左上
            (cx + half_length, cy - half_width),  # 右上
            (cx + half_length, cy + half_width),  # 右下
            (cx - half_length, cy + half_width),  # 左下
        ]
        return vertices
    ...
    def enter(self):
        ...
        if state == READY_NAVIGATE:
            ...
            # 进入环绕状态，开始环绕目标物体
            if self.move_direction == D_TO_D:
                # 主车顺时针旋转
                self.orbit_angle_buf = -self.my_vision.orbit_angle
            elif self.move_direction == U_TO_U:
                # 主车逆时针旋转
            ...
            elif self.move_direction in [U_TO_U, D_TO_U]:
                # 搬运到260的点保证光电管能检测到边界
        ...
        if state == READY_NAVIGATE:
            ...
            if not self.my_plan.if_finish_navigate:
                ...
                if self.move_direction == D_TO_D:
                    ...
                    self.data.scan_point[1][0] = self.my_car.x_current 
                ...
                y_th = 20.0
                rich_angle = 8.0  # 角度裕量
                rect_vertices = self.get_rect_vertices(self.predict_message, x_th, y_th)   
                if self.move_direction in [D_TO_D, D_TO_U]:
                    self.spin_angle_buf = -180 + self.my_vision.orbit_angle + rich_angle

                    if self.move_direction == D_TO_D:
                        slave_angle = 0.0
                    elif self.move_direction == D_TO_U:
                        slave_angle = 180 - self.my_vision.orbit_angle - rich_angle
                    
                    self.my_main_protocol.send_path(self.current_object, slave_angle, rect_vertices[3])   
                    
                    self.predict_message = rect_vertices[2]
                else:
                    self.spin_angle_buf = self.my_vision.orbit_angle + rich_angle

                    if self.move_direction == U_TO_U:
                        slave_angle = 180.0
                    elif self.move_direction == U_TO_D:
                        slave_angle = -self.my_vision.orbit_angle - rich_angle

                    # 让从车停靠在主车左侧伺服后不环绕直接往下边界搬运    
                    self.my_main_protocol.send_path(self.current_object, slave_angle, rect_vertices[0])
                    
                    self.predict_message = rect_vertices[1]
            ...
            if self.data.finished_num >= self.data.total_objects_num:
                ...
                if self.task_mode == TASK_CRAZY:
                    if self.move_direction == D_TO_U:
                        self.move_direction = U_TO_D
                    elif self.move_direction == U_TO_D:
                        # 现在是最后一个物体
                        if self.data.finished_num == self.data.total_objects_num - 1:
                            self.move_direction = D_TO_D
                        else:
                            self.move_direction = D_TO_U
                elif self.task_mode == TASK_NORMAL:
                    if self.move_direction == D_TO_U:
                        if self.data.finished_num == self.data.total_objects_num - 1:
                            self.move_direction = U_TO_D
                        else:
                            self.move_direction = U_TO_U
                    elif self.move_direction == U_TO_U:
                        if self.data.finished_num == self.data.total_objects_num - 1:
                            self.move_direction = U_TO_D
                self.my_plan.reset_navigate_angle()
    ...
    def handle_ready_navigate(self):
        ...
        stop_threshold_x = 15.0  # 从车停止点与主车的x距离阈值
        stop_threshold_y = 25.0  # 从车停止点与主车的y距离阈值
        if self.move_direction in [D_TO_D, D_TO_U]:
            target_angle = 0.0
            target_point = self.data.scan_point[0]
            self.scan_message = self.data.scan_path_1
            self.slave_navigate_message = [[target_point[0] - stop_threshold_x, target_point[1] - stop_threshold_y], target_angle]
        else:
            target_angle = 180.0
    ...
    def handle_scan(self):
        ...
        if_pos_rational = self.task_mode == TASK_CRAZY or (self.move_direction != D_TO_D) or\
        (self.my_car.x_current <= 145.0 or self.my_car.x_current >= 175.0)
        
        target_point = self.my_art_protocol.coordinate_receive()
        if target_point and if_pos_rational and target_point[1] >= 10:
            # 更新当前伺服物体种类
            self.my_vision.current_servo_object = chr(target_point[2])  
            # 判断目标点是否合理，若不合理则清空当前伺服物体种类    
            if  self.task_mode == TASK_CRAZY or (self.move_direction != D_TO_D) or \
                self.my_vision.judge_if_object_rational(target_point[0], target_point[1]):
