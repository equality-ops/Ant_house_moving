...
static uint8 put_char(char *buf, uint8 len, uint8 *pos, char c)
{
...
static uint8 put_u32(char *buf, uint8 len, uint8 *pos, uint32 value)
{
    char tmp[10];
    uint8 n = 0;

    if (value == 0) {
        return put_char(buf, len, pos, '0');
    }
    ...
    while (n > 0) {
        n--;
        if (!put_char(buf, len, pos, tmp[n])) return 0;
    }

    return 1;
}

char *k251_float_to_str(float value, char *buf, uint8 len, uint8 precision)
{
    uint8 pos = 0;
    uint8 i;
    uint32 scale = 1;
    ...
    if (value < 0.0f) {
        if (!put_char(buf, len, &pos, '-')) return 0;
        value = -value;
    }
    ...
    scaled = (uint32)(value * (float)scale + 0.5f);
    int_part = scaled / scale;
    frac_part = scaled % scale;

    if (!put_u32(buf, len, &pos, int_part)) return 0;
    ...
    if (precision > 0) {
        ...
        for (i = 0; i < precision; i++) {
            if (!put_char(buf, len, &pos, (char)('0' + (frac_part / div) % 10))) {
                return 0;
            }
            div /= 10;
        }
